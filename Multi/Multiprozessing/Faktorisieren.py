...
def mp_factor(numbers, processes):
    ...
    for i in range(processes):
        proc = multiprocessing.Process(target=mp_worker, args=(
            queue, numbers[chunks*i:chunks*(i+1)]))
        procs.append(proc)
        proc.start()
...
if __name__ == '__main__':
    ...
    for procs in [1, 2, 4, 8]:
        start = time.time()
        result = mp_factor(numbers, procs)
        print("Multiprocessing with " + str(procs) +
              " Processes took " + str((time.time() - start)) + " seconds.")
    for threads in [1, 2, 4, 8]:
        start = time.time()
        result = mt_factor(numbers, threads)
        print("Multithreading with " + str(threads) +
              " Threads took " + str((time.time() - start)) + " seconds.")
"""
randint()
